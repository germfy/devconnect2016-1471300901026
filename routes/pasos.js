...
function enviarPush(token, texto, userID, respuesta, cuerpo){
  request({
    ...
    if(error) {
        ...
        console.log("Codigo de respuestas" + response.statusCode);
        respuesta = response;
        cuerpo = body;
    }
  });
...
router.get('/paso1', function(req, res, next){
  ...
  var respuesta, cuerpo;
  enviarPush(req.token, "Prueba de paso 1", req.userID,  respuesta, cuerpo, function(req, res, next){
    res.send(respuesta);
  }
  );
});
